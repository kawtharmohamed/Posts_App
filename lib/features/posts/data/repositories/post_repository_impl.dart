...
typedef Future<Unit> DeleteOrAddOrUpatePost();

class PostsRepositoryImpl implements PostsRepository {
  final PostRemoteDataSource remoteDataSource;
  final PostLocalDataSource localDataSource;
  final NetworkInfo networkInfow;

  PostsRepositoryImpl(
      {required this.remoteDataSource,
      required this.localDataSource,
      required this.networkInfow});

  @override
  Future<Either<Failure, List<Post>>> getAllPosts() async {
    if (await networkInfow.isConnected) {
      try {
  ...
  Future<Either<Failure, Unit>> updatePost(Post post) async {
    final PostModel postModel =
        PostModel(title: post.title, id: post.id, body: post.body);
    return await getMessage(() {
  ...
  Future<Either<Failure, Unit>> getMessage(
      DeleteOrAddOrUpatePost deleteOraddOrupdatePost) async {
    if (await networkInfow.isConnected) {
      try {
